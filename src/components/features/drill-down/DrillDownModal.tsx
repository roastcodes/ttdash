      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="grid grid-cols-4 gap-3 text-center">
          <div className="p-2 rounded-lg bg-muted/30">
            ...
            <div className="font-mono font-medium">{formatTokens(day.totalTokens)}</div>
          </div>
          <div className="p-2 rounded-lg bg-muted/30">
            <div className="text-xs text-muted-foreground">$/1M</div>
            <div className="font-mono font-medium">${(day.totalCost / (day.totalTokens / 1_000_000)).toFixed(2)}</div>
          </div>
          <div className="p-2 rounded-lg bg-muted/30">
            <div className="text-xs text-muted-foreground">Cache-Rate</div>
            <div className="font-mono font-medium">{formatPercent(cacheRate)}</div>
          </div>
          <div className="p-2 rounded-lg bg-muted/30">
            ...
            {modelData.map(model => (
              <div key={model.name} className="flex items-center justify-between text-sm p-1.5 rounded hover:bg-muted/30">
                ...
                <div className="text-right font-mono">
                  ...
                  <span className="text-muted-foreground ml-2 text-xs">{formatTokens(model.tokens)}</span>
                </div>
              </div>
            ))}
          </div>
