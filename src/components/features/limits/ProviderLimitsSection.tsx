...
export function ProviderLimitsSection({ data, providers, limits, selectedMonth }: ProviderLimitsSectionProps) {
  ...
  const {
    ...
    const nextRows: ProviderLimitRow[] = providers.map((provider) => {
    ...
    const nextTimeline = months.map((month) => {
      ...
      const totalCost = providers.reduce((sum, provider) => sum + (monthCosts.get(provider) ?? 0), 0)
    ...
    <div id="limits" ref={sectionRef}>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-3">
        {rows.map((row, index) => {
          ...
          const riskProgress = row.monthlyLimit > 0 ? Math.min((row.cost / row.monthlyLimit) * 100, 100) : 0
          const subscriptionProgress = row.hasSubscription && row.subscriptionPrice > 0
          ...
          return (
            ...
            >
              <Card className={row.riskStatus === 'limit' ? 'border-red-500/40 bg-red-500/[0.06]' : row.subscriptionStatus === 'gain' ? 'border-emerald-500/30 bg-emerald-500/[0.05]' : undefined}>
                <CardContent className="p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-0.5 text-xs text-muted-foreground/80">{subscriptionLabel(row)}</div>
                    </div>
                    <div
                      className="rounded-full border px-2.5 py-1 text-[11px] font-medium"
                      style={providerStyle}
                    >
                      {row.monthlyLimit > 0 ? `${row.utilization?.toFixed(0)}% Limit` : row.hasSubscription ? `${Math.min(subscriptionProgress, 999).toFixed(0)}% Sub` : 'Offen'}
                    </div>
                  </div>
                  ...
                  <div className="mt-4 space-y-3">
                    ...
                    <div>
                      ...
                      <div className="h-2 overflow-hidden rounded-full bg-muted/30">
                        {row.hasSubscription ? (
                          <motion.div
                            className={row.subscriptionStatus === 'gain' ? 'h-full bg-emerald-400' : 'h-full bg-amber-300'}
                            initial={{ width: 0 }}
                            animate={inView ? { width: `${Math.max(8, subscriptionProgress)}%` } : { width: 0 }}
                            transition={{ duration: 0.8, delay: 0.12 + index * 0.04, ease: 'easeOut' }}
                          />
                        ) : (
                          <div className="h-full w-full bg-muted/20" />
                        )}
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-2">
        ...
        >
          <div className="space-y-3">
            {rows.map((row, index) => {
              const maxValue = row.monthlyLimit > 0
                ? Math.max(row.monthlyLimit, row.cost, 1)
                : Math.max(row.cost, 1)
              const scaleMax = maxValue * 1.15
              const costWidth = `${(row.cost / scaleMax) * 100}%`
              const limitPosition = row.monthlyLimit > 0 ? `${(row.monthlyLimit / scaleMax) * 100}%` : '0%'
              const withinLimitWidth = row.monthlyLimit > 0 ? `${(Math.min(row.cost, row.monthlyLimit) / scaleMax) * 100}%` : costWidth
              const overLimitWidth = row.monthlyLimit > 0 && row.overrun > 0
                ...
                >
                  ...
                  <div className="mt-4">
                    <div className="relative h-14">
                      ...
                      {row.monthlyLimit > 0 ? (
                        <>
                          <div className="absolute left-0 top-5 h-4 rounded-l-full bg-sky-400/12" style={{ width: limitPosition }} />
                          <div className="absolute top-5 h-4 rounded-r-full bg-red-400/12" style={{ left: limitPosition, width: `calc(100% - ${limitPosition})` }} />

                          <motion.div
                            className={row.riskStatus === 'warning' ? 'absolute left-0 top-5 h-4 rounded-full bg-amber-400' : 'absolute left-0 top-5 h-4 rounded-full bg-sky-400'}
                            initial={{ width: 0 }}
                          ...
                          {row.overrun > 0 && (
                            <motion.div
                              ...
                              transition={{ duration: 0.75, delay: 0.14 + index * 0.04, ease: 'easeOut' }}
                            />
                          )}
                      ...
                      <div className="absolute top-11 left-0 text-[10px] text-muted-foreground">$0</div>
                      <div className="absolute top-11 right-0 text-[10px] text-muted-foreground">{formatCurrency(scaleMax)}</div>
                    </div>
                  </div>
        ...
        >
          <div className="space-y-3">
            {rows.map((row, index) => {
              ...
              const scaleMax = maxValue * 1.15
              const costWidth = `${(row.cost / scaleMax) * 100}%`
              const subPosition = row.hasSubscription ? `${(row.subscriptionPrice / scaleMax) * 100}%` : '0%'
              const withinSubscriptionWidth = row.hasSubscription ? `${(Math.min(row.cost, row.subscriptionPrice) / scaleMax) * 100}%` : costWidth
              const overSubscriptionWidth = row.hasSubscription && row.subscriptionGain > 0
                ...
                >
                  ...
                  <div className="mt-4">
                    <div className="relative h-14">
                      ...
                      {row.hasSubscription ? (
                        <>
                          <div className="absolute left-0 top-5 h-4 rounded-l-full bg-amber-300/18" style={{ width: subPosition }} />
                          <div className="absolute top-5 h-4 rounded-r-full bg-emerald-400/14" style={{ left: subPosition, width: `calc(100% - ${subPosition})` }} />
