...
const HELP_CONTENT = {
  de: {
    ...
    chart: {
      ...
      requestCacheHitRate: 'Zeigt die Cache-Hit-Rate pro Modell zusammen mit dem gefilterten Gesamtwert und dem gleitenden 7-Tage-Durchschnitt auf Basis des gewählten Tagesbereichs.',
  ...
  en: {
    ...
    metric: {
      totalCost: 'Shows total API cost across the selected range. The calculation is based on configured per-model token prices.',
      totalTokens: 'Shows the sum of all processed tokens across input, output, and cache. One token is roughly four text characters.',
      ...
      cacheHitRate: 'Shows the share of tokens served from cache. Higher values usually indicate better cost efficiency.',
      costPerMillion: 'Shows average cost per 1 million processed tokens. Lower values indicate more efficient usage.',
      ...
      outputTokens: 'Shows the volume of generated output tokens. These are usually more expensive than pure input tokens.',
    },
    chart: {
      costOverTime: 'Shows API cost over time together with a rolling 7-day average. Clicking a point opens the drilldown.',
      costByModel: 'Shows cost distribution by model as a donut chart. This makes the main cost drivers visible.',
      costByModelOverTime: 'Shows how cost per model evolves over time. Useful for spotting shifts in the model mix.',
      cumulativeCost: 'Shows cumulative total cost over the selected range. When possible, a month-end projection is added.',
      costByWeekday: 'Shows average cost by weekday, making recurring weekly load patterns visible.',
      tokensOverTime: 'Shows token usage over time split by input, output, cache write, cache read, and thinking.',
      requestsOverTime: 'Shows requests over time with total line, per-model lines, and a trend line. Clicking a point opens the drilldown.',
      requestCacheHitRate: 'Shows cache hit rate per model together with the filtered overall total and the trailing 7-day average based on the selected daily range.',
      tokenTypes: 'Shows the distribution of token types as a donut chart so input, output, cache, and thinking shares become visible.',
      tokenEfficiency: 'Shows cost per 1 million tokens over time. This helps spot whether model mix and cache usage are becoming more or less efficient.',
      modelMix: 'Shows the percentage cost share of models at each period point. This makes model shifts and concentration visible.',
      distributionAnalysis: 'Shows histograms for costs, requests, and tokens per request. This highlights spread, not just averages.',
      correlationAnalysis: 'Shows scatter plots for potential relationships such as requests to cost or cache rate to cost per request. Correlation is a signal, not proof of causality.',
      heatmap: 'Shows a calendar heatmap of daily cost. Darker cells indicate higher values.',
      requestHeatmap: 'Shows a calendar heatmap of requests per day. This reveals load patterns independently of cost.',
      tokenHeatmap: 'Shows a calendar heatmap of token volume per day. This helps distinguish high-volume days from high-cost days.',
      forecast: 'Shows cost forecast for the current month based on smoothed calendar-day costs, complemented by trend and uncertainty band.',
      cacheROI: 'Shows the impact of cache usage by comparing hypothetical no-cache costs with actual costs.',
      providerLimitProgress: 'Shows how much of each configured monthly provider limit has already been used. Overruns are marked separately.',
      providerSubscriptionMix: 'Compares fixed subscription cost with variable API cost per provider and can optionally include the configured monthly limit.',
      providerLimitTimeline: 'Shows monthly provider cost against total configured limits over time so bottlenecks become visible early.',
      periodComparison: 'Shows the comparison of two periods, such as week-over-week or month-over-month, using central metrics.',
      anomalyDetection: 'Shows unusual period points with unusually high or low cost based on deviation from the mean in standard deviations.',
    },
    section: {
      insights: 'Shows condensed statements about concentration, request economics, usage patterns, and peak windows. This section is meant as a fast entry point before deeper analysis.',
      metrics: 'Shows the most important KPIs at a glance. Hover abbreviated values to see exact numbers.',
      today: 'Shows the KPIs of the current day in the dataset. This makes it easy to compare the day against the period average.',
      currentMonth: 'Shows the KPIs of the current month, including progress, coverage, and comparison to the previous month.',
      activity: 'Shows calendar views for cost, requests, and tokens, making spikes, gaps, and seasonal patterns visible.',
      forecastCache: 'Shows month forecast, cache savings, and operational request quality in one block for a combined outlook on efficiency.',
      limits: 'Shows configured subscriptions and monthly limits per provider. This section makes it visible how far budgets are stretched in the current slice.',
      costAnalysis: 'Shows cost trend and cost distribution by model so it is clear where spend happened and which models dominate.',
      tokenAnalysis: 'Shows token volume, token types, weekday patterns, and efficiency so you can judge whether cost comes from volume or pricing level.',
      requestAnalysis: 'Shows requests overall, by model, and over time. The expanded views add extra trends and distributions.',
      advancedAnalysis: 'Shows distributions, correlations, and concentration risk so usage stability, concentration, and unusual behavior become visible.',
      comparisons: 'Shows changes between periods and marks outliers so shifts can be understood faster.',
      tables: 'Shows detailed tables with sorting and drilldown so individual models, providers, and days can be inspected directly.',
    },
    feature: {
      ...
      recentDays: 'Shows the detailed table per day, month, or year with drilldown and benchmarks against the previous day and 7-day average.',
    },
  },
} as const
