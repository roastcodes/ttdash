...
#box(fill: rgb("#f0f6ff"), inset: 16pt, radius: 18pt, width: 100%)[
  #align(left)[
    ...
    #grid(
      columns: (1fr, 1fr, 1fr),
      gutter: 8pt,
      metric-card("Zeitraum", report.labels.dateRangeText),
      metric-card("Ansicht", report.meta.filterSummary.viewMode),
      metric-card("Generiert", report.meta.generatedAtLabel),
    )
  ]
]
...
#grid(
  columns: (1fr, 1fr, 1fr),
  gutter: 8pt,
  metric-card("Monat", if report.meta.filterSummary.selectedMonthLabel != none { report.meta.filterSummary.selectedMonthLabel } else { "Alle" }),
  metric-card("Ausgewählte Provider", if report.meta.filterSummary.selectedProviders.len() > 0 { report.meta.filterSummary.selectedProviders.join(", ") } else { "Alle" }),
  metric-card("Ausgewählte Modelle", if report.meta.filterSummary.selectedModels.len() > 0 { report.meta.filterSummary.selectedModels.join(", ") } else { "Alle" }),
  metric-card("Startdatum", if report.meta.filterSummary.startDateLabel != none { report.meta.filterSummary.startDateLabel } else { "Kein Filter" }),
  metric-card("Enddatum", if report.meta.filterSummary.endDateLabel != none { report.meta.filterSummary.endDateLabel } else { "Kein Filter" }),
)
...
#grid(
  ...
  gutter: 10pt,
  rect(inset: 10pt, radius: 14pt, fill: panel, stroke: (paint: line, thickness: 0.8pt))[
    #text(size: 12pt, weight: "bold", fill: ink)[Top-Modelle]
    #v(6pt)
    #set text(size: 8.8pt)
    #table(
      ...
      table.header([*Modell*], [*Provider*], [*Kosten*], [*Requests*]),
  ...
  rect(inset: 10pt, radius: 14pt, fill: panel, stroke: (paint: line, thickness: 0.8pt))[
    #text(size: 12pt, weight: "bold", fill: ink)[Provider]
    #v(6pt)
    #set text(size: 8.8pt)
    #table(
      ...
      table.header([*Provider*], [*Kosten*], [*Tokens*], [*Requests*]),
...
#rect(inset: 10pt, radius: 14pt, fill: panel, stroke: (paint: line, thickness: 0.8pt))[
  ...
  #table(
    ...
    column-gutter: 8pt,
    align: (x, y) => if x == 0 { left } else { right },
    table.header([*Zeitraum*], [*Kosten*], [*Tokens*], [*Requests*]),
...
function createChartAssets(reportData) {
  const costTrend = reportData.charts.costTrend.map((entry) => ({
    label: formatDateAxis(entry.date),
    cost: entry.cost,
    ma7: entry.ma7 || 0,
  }));
