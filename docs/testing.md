...
- The main CI workflow is intentionally a DAG: `static`, Vitest project matrix, `coverage`, `build`, and the clean `bun-toolchain` compatibility job can run independently; `package-smoke` and `e2e` depend only on the `production-dist` artifact from `build`.
