...
const { formatAttachmentDisposition, getErrorMessage, sendSSE } =
  require('../../server/routes/http-route-utils.js') as {
    formatAttachmentDisposition: (filename: string | null, fallback: string) => string
...
describe('HTTP route utilities', () => {
  it('formats attachment filenames with one consistent fallback', () => {
    expect(formatAttachmentDisposition(null, 'fallback.json')).toBe(
      'attachment; filename="fallback.json"; filename*=UTF-8\'\'fallback.json',
    )
