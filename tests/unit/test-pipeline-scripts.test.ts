...
describe('test pipeline scripts', () => {
  ...
  it('plans automatic releases without secrets before entering the release environment', async () => {
    const releaseWorkflow = await readFile('.github/workflows/release.yml', 'utf8')
    const verifyScript = await readFile('scripts/verify-main-ci.js', 'utf8')
    const bunVersion = (await readFile('.bun-version', 'utf8')).trim()
    const prepareBlock = getWorkflowJobBlock(releaseWorkflow, 'prepare')
    const releaseBlock = getWorkflowJobBlock(releaseWorkflow, 'release')

    expect(releaseWorkflow).toContain("cron: '30 12 * * *'")
    expect(prepareBlock).toContain('name: Plan release')
    expect(prepareBlock).toContain('permissions:')
    expect(prepareBlock).toContain('actions: read')
    expect(prepareBlock).toContain('contents: read')
    expect(prepareBlock).toContain('pull-requests: read')
    expect(prepareBlock).not.toContain('environment: release')
    expect(prepareBlock).not.toContain('id-token: write')
    expect(prepareBlock).toContain('node scripts/plan-release.js')
    expect(prepareBlock).toContain('--quiet-hours 2')
    expect(prepareBlock).toContain('--workflow ci.yml')
    ...
    expect(releaseBlock).toContain('needs: prepare')
    expect(releaseBlock).toContain("if: needs.prepare.outputs.should_release == 'true'")
    expect(releaseBlock).toContain('environment: release')
    expect(releaseBlock).toContain('id-token: write')
    expect(releaseBlock).toContain('contents: read')
    expect(releaseBlock).toContain('ref: ${{ env.RELEASE_TARGET_SHA }}')
    ...
    expect(releaseBlock).toContain('git push --atomic origin')
    expect(releaseBlock).toContain('REMOTE_MAIN_SHA')
    expect(releaseBlock).toContain('main moved from ${RELEASE_TARGET_SHA}')
    expect(releaseBlock.indexOf('- name: Create release app token')).toBeGreaterThan(
      releaseBlock.indexOf('- name: Run documentation browser and accessibility tests'),
    )
    expect(releaseBlock.indexOf('- name: Configure authenticated remote')).toBeGreaterThan(
      releaseBlock.indexOf('- name: Create release app token'),
    )
    expect(bunVersion).toBe('1.3.14')
    expect(verifyScript).toContain("url.searchParams.set('event', options.event)")
    expect(verifyScript).toContain("url.searchParams.set('head_sha', options.sha)")
